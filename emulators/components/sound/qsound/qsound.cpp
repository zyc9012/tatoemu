...
// Adapted from FBNeo implementation by superctr (Ian Karlsson)
...
static INT32 sample_rom_size = 0;
...
static const INT16 qsound_filter_data2[] = {
...
inline INT16 get_sample(UINT16 bank, UINT16 address)
{
	...
	sample_data = sample_rom[rom_addr & rom_mask];
...
void QscSetSampleROM(UINT8* rom, INT32 size)
{
	...
	sample_rom = (UINT8*)BurnMalloc(size);
	memcpy(sample_rom, rom, size);
...
// Get current left sample
INT16 QscGetLeftSample()
{
	return chip.out[0];
}

// Get current right sample
