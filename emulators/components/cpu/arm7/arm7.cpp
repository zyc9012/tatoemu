...
// Register access functions for HLE BIOS and save states
UINT32 Arm7GetRegister(int reg) {
	int mode = ARM7REG(eCPSR) & 0xF;
	if (reg == 16) return ARM7REG(eCPSR);
	if (reg == 17) return ARM7REG(sRegisterTable[mode][17]);
	return ARM7REG(sRegisterTable[mode][reg]);
}

void Arm7SetRegister(int reg, UINT32 value) {
	int mode = ARM7REG(eCPSR) & 0xF;
	if (reg == 16) { ARM7REG(eCPSR) = value; return; }
	if (reg == 17) { ARM7REG(sRegisterTable[mode][17]) = value; return; }
	ARM7REG(sRegisterTable[mode][reg]) = value;
}

UINT32 Arm7GetCPSR() {
	return ARM7REG(eCPSR);
}
...
UINT32 Arm7GetSPSR() {
	int mode = ARM7REG(eCPSR) & 0xF;
	return ARM7REG(sRegisterTable[mode][17]);
}
