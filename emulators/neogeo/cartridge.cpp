...
Cartridge::Cartridge()
    ...
    , m_isAES(false)                 // Default to MVS system
    , m_programRomSize(0)
    , m_spriteRomSize(0)
    , m_textRomSize(0)
    , m_soundRomSize(0) {
...
bool Cartridge::loadROMsFromDatabase(const std::map<std::string, std::vector<u8>>& romFiles) {
            ...
            if (GameDatabase::validateROM(pair.first, pair.second, entry)) {
                ...
                switch (entry.type) {
                    ...
                    case ROMType::SPRITE:
                        ...
                        spriteRomSizes.push_back(entry.size);
                        break;
    ...
    // Handle sprite ROM interleaving
    {
        u32 totalSize = 0;
    ...
    // Decode ROMs (text and sprite graphics need preprocessing)
    decodeTextROM();
    decodeSpriteROM();
...
void Cartridge::decodeTextROM() {
    ...
    u32 tileCount = m_textRomSize / 32;
...
void Cartridge::decodeSpriteROM() {
    ...
    u32 spriteCount = m_spriteRomSize / 128;
